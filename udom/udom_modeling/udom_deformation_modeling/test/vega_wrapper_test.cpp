...
namespace
{
class VegaWrapperTest : public ::testing::Test
{
protected:
    virtual void SetUp()
    {
    ...
    const std::string mesh_filename = "test_mesh_0_indexed.veg";
...
TEST_F(VegaWrapperTest, test_init_vertices)
{
...
TEST_F(VegaWrapperTest, test_init_elements)
{
    my_interface->init_mesh(ros_mesh);
...
TEST_F(VegaWrapperTest, test_mesh_dofs)
{
    int mesh_dofs = my_interface->mesh_dofs();
    EXPECT_EQ(24, mesh_dofs);
}

TEST_F(VegaWrapperTest, test_extract_points)
{
    std::vector<int> indices = {0, 5, 7};
    visualization_msgs::Marker points;
    my_interface->extract_points(indices, points, 0);
...
TEST_F(VegaWrapperTest, test_extract_points_various_dofs)
{
...
TEST_F(VegaWrapperTest, test_get_displacements)
{
    int mesh_dofs = my_interface->mesh_dofs();
